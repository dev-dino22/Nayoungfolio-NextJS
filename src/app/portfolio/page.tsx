...
import { useNavMenu } from "../../context/NavContext";
import { useWindowSize } from "../../context/WindowSizeContext"; // WindowSizeContext에서 가져오기
...
const PortfolioUI = () => {
    ...
    const { width, height } = useWindowSize(); // 전역 상태에서 width와 height 가져오기
    const { botHeight } = useNavMenu();
    ...
    const keyRef1 = useRef<SVGPathElement>(null);
    ...
    useEffect(() => {
        if (filterKey) {
            setSelectedKeys([filterKey as string]); // 쿼리에서 filterKey 가져오기
        }
    }, [filterKey]);

    useEffect(() => {
        setIsMobile(width < 991); // width를 사용하여 모바일 여부 설정
    }, [width]);
    ...
    return (
        <Container height={botHeight} className=".container">
            <Wrap>
                {!isMobile && <Background>
                    {firstHand && <HandDragInfo>
                        <img src="image/portfolio/mouseSVG.svg"></img>
                    </HandDragInfo>}
                    <TitleWrap>
                        <Title>Works.</Title>
                        <SubWrap>
